...
public class BotCore {
  ...
  private void startUp() {
    ...
    Runtime.getRuntime().addShutdownHook(new Thread(() -> {
      audioManager.getLavalinkManager().getLavalink().getLinks().forEach(Link::destroy);
    ...
    try {
      ...
      .addEventListeners(
          shardStartListener,
          messageListener,
          audioManager.getLavalinkManager().getLavalink()
      )
      .setVoiceDispatchInterceptor(audioManager.getLavalinkManager().getLavalink().getVoiceInterceptor())
      .addEventListenerProvider(id -> this.getShard(id).getListener());
      ...
      log.error("Failed on login operation");
  ...
  private Shard getShard(int id) {
    return botManager.getShardsMap().computeIfAbsent(id, Shard::new);
  }
